// Smets & Wouters (2007) Replication with no growth assumption
// we take gamma_bar = 0
...
parameters
    ...
    Mu_w Mu_p alpha psi phi delta sigma_c lambda
    phi_p iota_w xi_w iota_p xi_p sigma_l phi_w 
    r_pi r_dy r_y rho 
    rho_a rho_b rho_g rho_i rho_r rho_p rho_w G;
...
model(linear); 
    ...
    w = (1/(1+beta_bar*gamma))*w(-1) + (beta_bar*gamma/(1+beta_bar*gamma))*w(1)
        + (iota_w/(1+beta_bar*gamma))*pinf(-1) - (1+beta_bar*gamma*iota_w)/(1+beta_bar*gamma)*pinf
        + (beta_bar*gamma)/(1+beta_bar*gamma)*pinf(1)
        + (1-xi_w)*(1-beta_bar*gamma*xi_w)/((1+beta_bar*gamma)*xi_w)*(1/((phi_w-1)*curv_w+1))
        * (sigma_l*l + (1/(1-lambda/gamma))*c - ((lambda/gamma)/(1-lambda/gamma))*c(-1) -w) + eps_w;
...
estimated_params;
    ...
    beta_const,0.7420,0.01,2.0,GAMMA_PDF,0.25,0.1;
